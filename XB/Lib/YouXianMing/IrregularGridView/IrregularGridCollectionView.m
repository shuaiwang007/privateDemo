...
- (instancetype)initWithFrame:(CGRect)frame flowLayout:(UICollectionViewFlowLayout *)flowLayout {
    
    if (self = [super initWithFrame:frame]) {
        
        self.contentEdgeInsets = UIEdgeInsetsMake(15, 15, 5, 5);
...
+ (instancetype)irregularGridCollectionViewWithFrame:(CGRect)frame
    ...
    IrregularGridCollectionView *irregularGridView = [[[self class] alloc] initWithFrame:frame flowLayout:flowLayout];
...
- (void)setRegisterCells:(NSArray <IrregularGridViewCellClassType *> *)registerCells {
    ...
    for (IrregularGridViewCellClassType *type in registerCells) {
        ...
        if (type.nibView) {
            [self.collectionView registerNib:[UINib nibWithNibName:type.reuseIdentifier bundle:nil] forCellWithReuseIdentifier:type.reuseIdentifier];
        } else {
